    ...
    private var list: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(alignment: .firstTextBaseline) {
                ...
                Spacer()
                OctetButton(title: "Close", kind: .ghost, compact: true) { AgentCenter.shared.board = nil }
                    .help("Back (Esc)")
            }
            ...
            .padding(.bottom, 10)
            HStack(spacing: 6) {
                OctetTextField(placeholder: "Describe a task for Codex to run", text: $task) { start() }
