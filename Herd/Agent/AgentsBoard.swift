    ...
    private var list: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(alignment: .firstTextBaseline) {
                ...
                Spacer()
                HerdButton(title: "Close", kind: .ghost, compact: true) { AgentCenter.shared.showingBoard = false }
                    .help("Back (Esc)")
            }
            ...
            .padding(.bottom, 10)
            HStack(spacing: 6) {
                HerdTextField(placeholder: "Describe a task for a new Claude background session", text: $task) { dispatch() }
                HerdButton(title: "Start", kind: .primary, compact: true) { dispatch() }
...
struct StateMark: View {
    ...
    var body: some View {
        ...
        case (.interactive, _):
            HerdIcon("terminal", size: 13).foregroundStyle(Theme.textTertiary)
        case (_, .needsInput):
            Circle().fill(Color(hex: "FFC107")).frame(width: 8, height: 8).padding(.top, 4)
        case (_, .working):
            LoadingLine(width: 12).padding(.top, 6)
        case (_, .done):
            HerdIcon("checkmark", size: 13).foregroundStyle(Theme.accent)
        case (_, .idle):
...
private struct AgentDetail: View {
    ...
    @ViewBuilder private var actions: some View {
        if agent.kind == .background {
            HerdButton(title: "Bring Back Here", kind: .primary, compact: true) {
            ...
            HerdButton(title: "Remove", kind: .ghost, compact: true) {
