...
enum SandboxStyle {
    ...
    /// Nil means whatever the person's own Codex config says, which Herd
...
/// Herd's effort panel: the level in its color, a slider across the levels
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if let level, EffortStyle.warned.contains(level) {
                HStack(alignment: .top, spacing: 6) {
                    HerdIcon("exclamationmark.triangle.fill", size: 13)
