...
extension TerminalEngine {
    ...
    class Config {
        ...
        /// Octet: build a config from defaults + the given config-file-syntax string.
        /// `ghostty_config_load_default_files` / CLI args are deliberately skipped so
        /// the user's personal terminal configuration files never leak into Octet.
        ...
        static func loadConfig(overrides: String) -> ghostty_config_t? {
            ...
            if !overrides.isEmpty {
                ...
                overrides.withCString { ptr in
                    ghostty_config_load_string(cfg, ptr, UInt(len), "octet")
                }
            }
    ...
    class App {
        ...
        /// Octet: applies new config-file-syntax overrides to the running app and
        ...
        /// reads. Octet has no such UI, so a paste is completed (confirmed) and an
        ...
        static func action(_ app: ghostty_app_t, target: ghostty_target_s, action: ghostty_action_s) -> Bool {
            ...
            case GHOSTTY_ACTION_CLOSE_WINDOW, GHOSTTY_ACTION_CLOSE_TAB, GHOSTTY_ACTION_QUIT:
                // Octet has one surface per host view: any close/quit request from
