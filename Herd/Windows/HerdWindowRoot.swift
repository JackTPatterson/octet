...
struct HerdWindowRoot: View {
    let spec: HerdWindowSpec
    ...
    init(spec: HerdWindowSpec, store: SessionStore, session: EngineSession?, slash: SlashController, prompt: PromptEditor) {
        self.spec = spec
        self.store = store
...
@MainActor
enum WindowOpener {
    static var open: ((HerdWindowSpec) -> Void)?
}
