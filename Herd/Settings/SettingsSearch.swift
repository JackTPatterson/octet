...
enum SettingsSearchIndex {
    ...
    private static let agents: [SettingsSearchEntry] = [
        ...
        .init(.agents, "Herd's slash command menu", ["slash", "/", "commands", "menu"]),
        .init(.agents, "Herd's command line", ["prompt", "editor", "command line", "history", "suggestion", "highlight", "autosuggest"]),
        ...
        .init(.agents, "Run commands from the menu", ["slash", "submit", "run", "command"]),
    ...
    private static let keyboard: [SettingsSearchEntry] = HerdShortcut.groups.flatMap { group in
...
struct SettingsSearchResults: View {
    ...
    var body: some View {
        if results.isEmpty {
            VStack(spacing: 8) {
                HerdIcon("magnifyingglass", size: 35)
...
private struct SearchResultRow: View {
    ...
    var body: some View {
        Button(action: action) {
            HStack(spacing: 10) {
                HerdIcon(entry.section.symbol, size: 15)
                    .foregroundStyle(Theme.textTertiary)
                    .frame(width: 16)
                ...
                HerdIcon("chevron.right", size: 14)
                    .foregroundStyle(Theme.textTertiary)
            }
...
/// Help menu items. Herd has no help book, so help is the README.
enum HerdHelp {
