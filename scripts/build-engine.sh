...
# Recorded Claude Code and Codex screens with the status each should read
# as, and an idle rule for Claude's prompt status line.
git -C "$build_dir/source" apply "$root/scripts/engine-status-corpus.patch"

(
    cd "$build_dir/source"
    CARGO_INCREMENTAL=0 CARGO_PROFILE_RELEASE_DEBUG=0 cargo build --release --locked
    # Some engine tests write to the state and config folders: keep them
    # off the real ones (they once replaced the cached Codex rules).
    scratch="$build_dir/xdg"
    mkdir -p "$scratch/state" "$scratch/config" "$scratch/data"
    export XDG_STATE_HOME="$scratch/state" XDG_CONFIG_HOME="$scratch/config" XDG_DATA_HOME="$scratch/data"
    cargo test --release --locked scrollbar_renders_only_a_capped_thumb
    cargo test --release --locked prompt_marks
    cargo test --release --locked bundled_manifests_read_recorded_screens
)
