...
/// Octet's own `/` menu for agent panes. Typing `/` on an empty prompt is
...
final class SlashController: ObservableObject {
    ...
    /// Called before the terminal sees a key. Returns true when Octet took it.
    ...
    /// off the main thread, plus what Octet itself knows: recent project
